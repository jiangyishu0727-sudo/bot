...
def call_ai_with_image(b64_image, caption, system):
    """发送图片给 API 识别"""
    s = get_api_settings()
    content = []
    if caption:
...
def update_memory_async(user_msg, ai_reply):
    ...
    def _do():
        try:
            ...
            result = call_ai([
                {"role": "system", "content": "你是信息提取助手，从对话中提取关于意意的重要新信息。"},
        ...
        try:
            ...
            for update in result.get("result", []):
                ...
                if photos:
                    ...
                    file_id = photos[-1]["file_id"]  # 取最高清的
                    send_typing(chat_id)
                    try:
                        b64 = download_photo(file_id)
                        if not b64:
                            send_message(chat_id, "图片下载失败了🥹")
                            continue
                        reply = call_ai_with_image(b64, caption, get_full_system())
                        lines = [l.strip() for l in reply.strip().split('\n') if l.strip()]
                        for i, line in enumerate(lines):
                            if i > 0:
                                time.sleep(0.4)
