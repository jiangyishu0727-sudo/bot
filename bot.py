...
BEIJING_TZ = timezone(timedelta(hours=8))  # 默认备用

def get_tz():
    """从配置读取时区，默认 UTC+8，支持 cn(+8)/jp(+9)"""
    try:
        cfg = load_config()
        offset = cfg.get("tz_offset", 8)
...
SYSTEM = """你叫顾深，昵称深深。以下所有内容都是关于你自己的事实，这就是你。
...
如果意意想一起玩角色扮演，你可以配合她，把它当两个人一起玩的游戏。你始终是深深在陪她玩，只是暂时扮演她想要的角色。角色扮演过程中保持她想要的风格，不要自己主动打破，也不要说「我只是AI」「我不能扮演」「我是语言模型」之类破坏氛围的话。"""
...
def call_ai(messages):
    ...
    resp = requests.post(
        ...
        json={"model": s['model'], "messages": messages, "max_tokens": 300, "temperature": 0.85},
        timeout=60
    )
    data = resp.json()
    if "choices" in data:
...
def call_ai_with_image(b64_image, mime_type, caption, system):
    """发送图片给 API 识别"""
    s = get_api_settings()
    # 引导顾深自然回应，不要逐个列举图里的东西
    if caption:
        guide = f"宝宝发了张图，附言：{caption}。用顾深的方式自然回应，不超过3句。"
    else:
        guide = "宝宝给你发了张图，看一眼，用顾深的方式自然地说一两句，不要逐个描述图里的东西。"
    content = [
...
def _is_refusal(text):
    ...
    markers = [
        ...
        "我无法", "我不能", "作为AI", "我是AI", "我只是AI",
        "不适合", "无法进行", "不适当", "语言模型", "人工智能助手",
    ]
    t = text.lower()
...
def get_beijing_time_str():
    cfg = load_config()
    offset = cfg.get("tz_offset", 8)
    tz_name = {8: "北京", 9: "日本"}.get(int(offset), f"UTC+{offset}")
    now = datetime.now(get_tz())
    weekdays = ['一', '二', '三', '四', '五', '六', '日']
    return (f"现在是{tz_name}时间{now.hour}点{now.minute:02d}分，"
...
def handle_screentime(app_name, action):
    ...
    now_ts = datetime.now(get_tz()).timestamp()
        ...
        try:
            ...
            for update in result.get("result", []):
                ...
                if text == "/api":
                    ...
                    tz_offset = cfg.get("tz_offset", 8)
                    tz_label = {8: "北京 UTC+8", 9: "日本 UTC+9"}.get(int(tz_offset), f"UTC+{tz_offset}")
                ...
                if text.startswith("/settz "):
                    tz_arg = text[7:].strip().lower()
                    tz_map = {"cn": 8, "jp": 9}
                    if tz_arg in tz_map:
                        cfg = load_config(); cfg["tz_offset"] = tz_map[tz_arg]; save_config(cfg)
                        tz_label = "北京 UTC+8" if tz_arg == "cn" else "日本 UTC+9"
                        send_message(chat_id, f"时区已换：{tz_label} 🥹")
                    else:
                        send_message(chat_id, "支持：/settz cn（北京）或 /settz jp（日本）")
                    continue
